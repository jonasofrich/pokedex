package com.example.pokedex.ui.viewmodel

import android.util.Log
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.pokedex.data.pokemon.PokemonRepository
import com.example.pokedex.domain.model.Pokemon
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.flow.update
import kotlinx.coroutines.launch
import javax.inject.Inject

sealed interface DetailsUiState {
    data object Loading : DetailsUiState
    data class Success(val pokemon: Pokemon) : DetailsUiState
    data class Error(val message: String) : DetailsUiState
}

@HiltViewModel
class RandomViewModel @Inject constructor(
    private val pokemonRepository: PokemonRepository
) : ViewModel() {

    private val _uiState = MutableStateFlow<DetailsUiState>(DetailsUiState.Loading)
    val uiState = _uiState.asStateFlow()

    fun toggleFavorite(id: Int) {
        viewModelScope.launch {
            pokemonRepository.toggleFavorite(id)
            _uiState.update {
                when (it) {
                    is DetailsUiState.Success -> it.copy(pokemon = it.pokemon.copy(isFavorite = !it.pokemon.isFavorite))
                    else -> it
                }
            }
        }
    }

    fun getRandomPokemon() {
        viewModelScope.launch {
            try {
                val randomPokemon = pokemonRepository.getRandomPokemon()
                _uiState.update { DetailsUiState.Success(pokemon = randomPokemon) }
            } catch (e: Exception) {
                Log.e("RandomViewModel", "Error fetching pokemon", e)
                _uiState.update { DetailsUiState.Error("Something went wrong") }
            }
        }
    }

    fun onScreenExit() {
        _uiState.update { DetailsUiState.Loading }
    }
}